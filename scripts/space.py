...
class RealParameter(Parameter) :
    ...
    def randomly_select(self):
        ...
        if isinstance(self.prior, list):
            ...
            sample = norm.rvs(loc=self.custom_gaussian_prior_mean[sample_idx], scale=self.custom_gaussian_prior_std[sample_idx])
            sample_count = 1
            while sample > self.max_value or sample < self.min_value:
                sample_count += 1
                if sample_count > 10000:
    ...
    def get_x_probability(self, x):
        ...
        if isinstance(self.prior, list):
            ...
            for idx in range(len(self.custom_gaussian_prior_mean)):
                prob += mixture_weights*norm.pdf(x, loc=self.custom_gaussian_prior_mean[idx], scale=self.custom_gaussian_prior_std[idx])
            return prob
        else:
            rescaled_x = self.from_parameter_value_to_0_1_range(x)
    ...
    def set_custom_gaussian_prior_params(self, mean, std, resample=False):
        if std == -1:
            std = (self.max_value - self.min_value)/2

        if resample:
            sample_std = 0.01*(self.max_value - self.min_value)
            if type(mean) == list:
                new_mean = []
                for mu in mean:
                    new_mu = stats.norm.rvs(loc=mu, scale=sample_std)
                    while new_mu > self.max_value or new_mu < self.min_value:
                        new_mu = stats.norm.rvs(loc=mu, scale=sample_std)
                    new_mean.append(new_mu)
            else:
                new_mean = stats.norm.rvs(loc=mean, scale=sample_std)
                while new_mean > self.max_value or new_mean < self.min_value:
                    new_mean = stats.norm.rvs(loc=mean, scale=sample_std)

            mean = new_mean
            #print("Changed prior mean:", new_mean)
            #print("Sampled with:", sample_std)
...
class Space :
    def __init__(self, config) :
        ...
        self.resample_mean = config["resample_mean"]

        bopro_mode = config['bopro_mode']["mode"]
        if (bopro_mode == 'exhaustive'):
            self.exhaustive_search_file = deal_with_relative_and_absolute_path("", config['bopro_mode']["exhaustive_search_file"])
        ...
        self.estimate_multivariate_gaussian = config["estimate_multivariate_gaussian"]
        if self.estimate_multivariate_gaussian:
            self.normalize_priors = True
            prior_estimation_file = config["prior_estimation_file"]
            point_quantile = config["prior_estimation_quantile"]
            data = pd.read_csv(prior_estimation_file, dtype=np.float64)
            prior_estimation_data = {}
            for key in data:
                prior_estimation_data[key] = data[key].tolist()

            all_input_data = np.array([self.convert_types_to_string(j, prior_estimation_data) for j in list(self.get_input_parameters())])
            threshold = np.quantile(prior_estimation_data[self.optimization_metrics[0]], point_quantile)
            indices = np.nonzero(prior_estimation_data[self.optimization_metrics[0]] <= threshold)[0]
            good_points = all_input_data[:,indices].T
            mu = np.mean(good_points,axis=0)
            means = config["custom_gaussian_prior_means"]
            for param_idx, input_param in enumerate(config["input_parameters"]):
                param_data = config["input_parameters"][input_param]
                mean = means[param_idx]
                if type(mean) == list:
                    mean = mean[0]
                param_min, param_max = param_data["values"]
                sampling_std = 0.01*(param_max - param_min)
                new_mean = stats.norm.rvs(loc=mean, scale=sampling_std)
                while new_mean > param_max or new_mean < param_min:
                    new_mean = stats.norm.rvs(loc=mean, scale=sampling_std)
                means[param_idx] = new_mean
            cov = (good_points - mu).T @ (good_points - mu) / good_points.shape[0]


            self.mv_mean = means

            diag = np.prod(np.diagonal(cov))
            geom_mean = diag**(1/len(mu))
            factor = sampling_std/geom_mean
            self.mv_cov = cov*factor

#            print("mu", self.mv_mean)
#            print("cov", self.mv_cov)


#            self.pdf = gaussian_kde(good_points, bw_method=self.bw_selector)

        # If using univariate kde priors, define prior for each parameter
        if not self.estimate_multivariate_gaussian and not self.estimate_prior_flag:
            for param_idx, param in enumerate(config["input_parameters"]):
                if config["input_parameters"][param]["prior"] == "estimate":
                    ...
                    if len(config["custom_gaussian_prior_stds"]) == 1:
                        ...
                        raise SystemExit
                    self.all_input_parameters[param].set_custom_gaussian_prior_params(mean, std, resample=self.resample_mean)
                elif config["input_parameters"][param]["prior"] == "gaussian_mixture":
                    self.normalize_priors = True
                    mean = config["custom_gaussian_prior_means"][param_idx]
                    std = config["custom_gaussian_prior_stds"][param_idx]
                    self.all_input_parameters[param].set_custom_gaussian_prior_params(mean, std, resample=self.resample_mean)
    ...
    def get_type(self, parameter) :
        """
        Get the BOPrO type of the parameter given as a input, i.e. "ordinal", "categorical", "integer" or "real".
        :param parameter: the parameter we want to get the BOPrO type from.
        :return: a string indicating the BOPrO type.
        """
    ...
    def get_random_configuration(self, use_priors=True) :
        ...
        if self.estimate_multivariate_gaussian:
            ...
            while valid_sample == False:
                configuration = {}
                valid_sample = True
                sample = multivariate_normal.rvs(mean=self.mv_mean, cov=self.mv_cov, size=1)
                for i, k in enumerate(input_parameters):
                    if (sample[i] >= input_parameter_objects[k].get_min()) and (sample[i] <= input_parameter_objects[k].get_max()):
                        ...
                        valid_sample = False
        elif estimate_prior_flag and use_priors:
            valid_sample = False
            while valid_sample == False:
                configuration = {}
                valid_sample = True
    ...
    def get_configuration_probability(self, configuration):
        ...
        elif self.estimate_multivariate_gaussian:
            conf_list = np.array([self.convert_types_to_string(j, configuration) for j in self.get_input_parameters()])
            return multivariate_normal.pdf(conf_list, mean=self.mv_mean, cov=self.mv_cov)
        else:
            raise SystemExit
    ...
    def convert_types_to_string(self, header, data_list) :
        ...
        This is useful when BOPrO wants to write back to file the configurations found in the original type format than the json file.
        :param header: the parameter name.
        :param data_list: the list to convert.
        ...
        for parameter in self.get_input_parameters():
            if parameter in configuration_input_parameters: # Double check that the parameter is available in configuration. There are cases in the code where we pass a configuration that only contains a subset of parameters of the original search space but we still want to compute an unique string for those cases.
                ...
                if isinstance(element, float) and element.is_integer():
                    element = int(element) # ensure consistency in how numbers are represented in the string
    ...
    def random_sample_array(self, all_data_array, beginning_of_time, number_of_RS=1000, debug=False):
        ...
        :param beginning_of_time: time from the beginning of the BOPrO design space exploration.
...
#            for param_index, cartesian_element in enumerate(cartesian_product):
    ...
    def run_configurations_client_server(self, beginning_of_time, configurations, run_directory, dontrun=False, doSleep=False,
        ...
        :param run_directory: the working directory where BOPrO is run (this is specified in the json file).
        :param dontrun:
        :param doSleep:
        :return:
        """
        ...
        if read_write_on_a_file :
            ...
            # this is part of the BOPrO/interacting system protocol
            ...
            # Write to stdout
            sys.stdout.write_protocol("Request %d\n" %len(configurations)) # From the BOPrO/interacting system protocol
            str_header = ""
            for parameter in list(input_parameters):
            ...
            if debug:
                # Just for testing without the need to run the system that is interacting with BOPrO on the other side.
                ...
                for header in self.get_input_and_output_parameters():
                    if header not in parameters_header:
                        ...
                        print("The key BOPrO was looking for is: %s" %str(header))
        ...
        for configuration in configurations:
            ...
            for param in configuration:
                if self.get_type(param) == "categorical":
                    tmp_configuration[param] = self.all_input_parameters[param].get_original_string(configuration[param])
                else:
    ...
    def run_configurations(
                        ...
                        batch_mode=False):
        """
        Run a set of configurations in one of BOPrOs modes.
        :param bopro_mode: which BOPrO mode to run as.
        :param configurations: a list of configurations (dict).
        :param beginning_of_time: time from the beginning of the BOPrO design space exploration.
        :param run_directory: the working directory where BOPrO is run (this is specified in the json file).
        :param black_box_function: objective function being optimized.
        ...
        :param run_directory: the directory where BOPrO is running.
        ...
        if (bopro_mode == 'default'):
            ...
            print("Unrecognized bopro mode:", bopro_mode)
            raise SystemExit
        return data_array
    ...
    def print_data_array(self, data_array):
        ...
        for idx in range(len(data_array[list(data_array.keys())[0]])):
            ...
            for key in data_array.keys():
                ...
                configuration += str(self.convert_types_to_string(key, data_array)[idx]) + ','
