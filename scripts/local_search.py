...
def get_neighbors(configuration, param_space):
    ...
    for param_name in input_parameters:
        ...
        if (param_type == 'categorical') or ((param_type == 'ordinal') and len(param_object.get_discrete_values()) <= 5):
            ...
            neighboring_values = stats.truncnorm.rvs(0, 1, loc=mean, scale=0.1, size=numeric_neighbors)
...
def run_objective_function(
    ...
    :param bopro_mode: which BOPrO mode is being used.
    :param param_space: a space object containing the search space.
    :param beginning_of_time: timestamp of when the optimization started.
    :param run_directory: directory where BOPrO is running.
...
def local_search(
    ...
    local_search_configurations = concatenate_data_dictionaries(local_search_configurations, param_mean, input_params)
...
def main(config, black_box_function=None, output_file=""):
    ...
    optimization_function_parameters = {}
    optimization_function_parameters['bopro_mode'] = bopro_mode
...
if __name__ == "__main__":
    ...
    if parameters_file == "--help" or len(sys.argv) != 2:
        ...
        print("### Example: ")
        print("### cd bopro")
        print("### python3 scripts/bopro.py example_scenarios/spatial/BlackScholes_scenario.json")
        print("################################################")
        raise SystemExit
