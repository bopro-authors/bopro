...
class RFModel(RandomForestRegressor):
        ...
        for tree_idx in range(number_of_trees):
            ...
            for leaf in samples_per_leaf.keys():
                if len(samples_per_leaf[leaf]) > 1:
                    ...
                    leaf_vars[leaf] = 0
                # leaf_vars[leaf] = max(leaf_vars[leaf], 0.01) # This makes BOPrO exploit too much. We will revisit this.
    ...
    def fit_RF(self, X, y, data_array=None, param_space=None, **kwargs):
        ...
        if (data_array is not None) and (param_space is not None):
            ...
            for tree_idx, tree in enumerate(self):
                samples_per_node = self.get_samples_per_node(tree, leaf_per_sample[tree_idx, :])
                ...
                for node_idx in range(tree.tree_.node_count):
                    ...
                    lower_bound, upper_bound = self.get_node_bounds(samples_per_node[node_idx], data_array[feature], threshold)
    ...
    def compute_rf_prediction(self, leaf_per_sample):
        ...
        for tree_idx in range(number_of_trees):
            for sample_idx in range(number_of_samples):
                ...
                sample_means[sample_idx] += tree_means_per_leaf[tree_idx][sample_leaf]/number_of_trees
        return sample_means
    ...
    def compute_rf_prediction_variance(self, leaf_per_sample, sample_means):
        ...
        for sample_idx in range(number_of_samples):
            for tree_idx in range(number_of_trees):
                sample_leaf = leaf_per_sample[tree_idx, sample_idx]
                mean_of_the_vars[sample_idx] += tree_vars_per_leaf[tree_idx][sample_leaf]/number_of_trees
                var_of_the_means[sample_idx] += (tree_means_per_leaf[tree_idx][sample_leaf]**2)/number_of_trees
...
def generate_mono_output_regression_models(data_array
        ...
        if objective_limits is not None:
            for col in Ycols:
                ...
                if objective_limits[col][1] == objective_limits[col][0]:
                    ...
                    normalized_col = [(x - objective_limits[col][0]) \
                                            /(objective_limits[col][1] - objective_limits[col][0]) for x in normalized_data_array[col]]
                normalized_data_array[col] = normalized_col
        else:
            sys.stdout.write_to_logfile("Warning: no limits provided, skipping objective normalization.\n")
    ...
    for i, Ycol in enumerate(Ycols):
        ...
        if model_type == "gaussian_process":
            ...
            with np.errstate(divide='ignore', over='ignore', invalid='ignore'): # GPy's optimize has uncaught warnings that do not affect performance, suppress them so that they do not propagate to BOPrO
            ...
            regressor[Ycol] = RFModel(n_estimators=n_estimators, max_features=max_features, bootstrap=bootstrap, min_samples_split=min_samples_split, **model_kwargs)
            regressor[Ycol].fit_RF(X_train, y_train, data_array=preprocessed_data_array, param_space=param_space)
            if print_importances:
...
def compute_recall_and_precision_on_RF_hyperparameters(dataset, param_space, X_train, y_train):
    ...
    The objective in BOPrO is to maximize the recall because we don't want to loose the opportunity of finding good samples.
    ...
    for score in scores:
        ...
        print("The model is trained on the BOPrO samples, total samples: %d" %len(X_train))
...
def compute_model_mean_and_uncertainty(bufferx, model, model_type, param_space, var=False):
    """
    ...
    if model_type == "random_forest":
        mean, uncertainty = compute_rf_prediction_mean_and_uncertainty(bufferx, model, param_space, var=var)
    if model_type == "gaussian_process":
        mean, uncertainty = compute_gp_prediction_mean_and_uncertainty(bufferx, model, param_space, var=var)
...
def compute_rf_prediction_mean_and_uncertainty(bufferx, model, param_space, var=False):
    ...
    for objective in model:
        leaf_per_sample = model[objective].get_leaves_per_sample(normalized_bufferx, param_space)
        prediction_means[objective] = model[objective].compute_rf_prediction(leaf_per_sample)
        prediction_variances = model[objective].compute_rf_prediction_variance(leaf_per_sample, prediction_means[objective])
        if var:
