...
def compute_probability_from_prior(configurations, param_space, objective_weights):
    ...
    if prior_estimation_flag or param_space.estimate_multivariate_gaussian:
...
def compute_EI_from_posteriors(
    ...
    for parameter in param_space.get_input_parameters():
        param_min, param_max = param_objects[parameter].get_min(), param_objects[parameter].get_max()
        for configuration in configurations:
...
def prior_guided_optimization(
    ...
    logfile = deal_with_relative_and_absolute_path(config["run_directory"], config["log_file"])
    ...
    eis, valids = compute_EI_from_posteriors([best_configuration_ls, best_configuration_cma], **function_parameters)
    if eis[0] < eis[1]:
